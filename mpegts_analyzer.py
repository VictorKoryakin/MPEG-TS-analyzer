...
binfolder=os.path.split(__file__)[0]+'/'  #определяем имя каталога в котором лежит наше ПО

#парсим команды запуска
...
#сооружаем сокет для прослушивания мультикаста
...
sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVTIMEO, timeval)    #вводим настройку (время ожидание входного пакета) сокета
...
sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq)  #нужно чтобы пограмма стала подписчиком на мультикаст, и отправляла igmp запрос
...
while True :   #основной цикл
    try:
        ...
        while data[-1:] == '\0': data = data[:-1] # Strip trailing \0's    Х.З. кажется подрезает нули в конце пакета, возможно нужно
        ...
        if time.time()-t > timeN :   #проверяем, если пора обработать результат, то
            ...
            print('----')
        if time.time()-tt > cc_reset_timer :    #если прошёл уже час, то сбрасываем счетчик ошибок в словаре
            tt=time.time()
            for keys in d.keys() :    #пробегаемся по всем пидам (записям словаря)
                d[keys][1]=0    #обнуляем накопленные в словаре ошибки
